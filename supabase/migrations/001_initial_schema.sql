-- CrisisLens Production Database Schema
-- Replace mock data with real persistent storage

-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "postgis";

-- Regions table with real geographical and infrastructure data
...
-- Incident reports with full details and geospatial data
CREATE TABLE incident_reports (
  ...
  sources JSONB DEFAULT '{}', -- Source information and citations
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Risk assessments with ML model data
CREATE TABLE risk_assessments (
  ...
  features JSONB NOT NULL DEFAULT '{}', -- Input features used for prediction
  feature_importance JSONB DEFAULT '{}', -- Feature importance from model
  prediction_timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  valid_until TIMESTAMP WITH TIME ZONE, -- When this assessment expires
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- External data feeds from APIs and services
CREATE TABLE external_data_feeds (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  source TEXT NOT NULL, -- 'openweather', 'emergency_services', 'news', 'social_media'
  data_type TEXT NOT NULL, -- 'weather', 'alert', 'news_article', 'social_post'
...
CREATE TABLE historical_incidents (
  ...
  severity INTEGER NOT NULL CHECK (severity BETWEEN 1 AND 5),
  affected_area_km2 FLOAT DEFAULT 0,
...
CREATE TABLE user_activity_logs (
  ...
  action TEXT NOT NULL, -- 'login', 'view_dashboard', 'submit_report', 'verify_report'
  resource_type TEXT, -- 'region', 'incident_report', 'risk_assessment'
  resource_id UUID,
...
CREATE INDEX idx_incident_reports_region_id ON incident_reports(region_id);
CREATE INDEX idx_incident_reports_created_at ON incident_reports(created_at DESC);
CREATE INDEX idx_incident_reports_severity ON incident_reports(severity DESC);
...
CREATE INDEX idx_risk_assessments_region_id ON risk_assessments(region_id);
...
CREATE INDEX idx_external_data_feeds_region_id ON external_data_feeds(region_id);
CREATE INDEX idx_external_data_feeds_source ON external_data_feeds(source);
CREATE INDEX idx_external_data_feeds_created_at ON external_data_feeds(created_at DESC);

CREATE INDEX idx_historical_incidents_region_id ON historical_incidents(region_id);
CREATE INDEX idx_historical_incidents_date ON historical_incidents(incident_date DESC);
...
-- Regions policy - anyone can read, only authenticated can modify
...
-- User activity logs policy - users can only see their own logs
